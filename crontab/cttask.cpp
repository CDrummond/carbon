...
QString CTTask::describe() const {
    ...
    if (reboot) {
        return QObject::tr("At system startup");
    }
    ...
    return QObject::tr("%1, %2", "1:Time Description, 2:Date Description").arg(timeFormat).arg(dateFormat);
}

QString CTTask::describeDayOfWeek() const {
    return QObject::tr("every %1", "Every 'days of week'").arg(dayOfWeek.describe());
}

QString CTTask::describeDayOfMonth() const {
    return QObject::tr("%1 of %2", "'Days of month' of 'Months'").arg(dayOfMonth.describe()).arg(month.describe());
}
...
QString CTTask::createDateFormat() const {
    ...
    if ((dayOfMonth.enabledCount() == CTDayOfMonth::MAXIMUM) && (dayOfWeek.enabledCount() == CTDayOfWeek::MAXIMUM)) {
        dateFormat = QObject::tr("every day ");
    }
    ...
    // Day of week not specified, so use day of month.
        ...
        dateFormat = QObject::tr("%1 as well as %2", "1:Day of month, 2:Day of week").arg(describeDayOfMonth()).arg(describeDayOfWeek());
    }
...
QString CTTask::describeDateAndHours() const {
        ...
        if (hour.isEnabled(h)) {
            for (int m = 0; m <= 59; m++) {
                if (minute.isEnabled(m) == true) {
                    ...
                    QString tmpStr = QObject::tr("%1:%2", "1:Hour, 2:Minute").arg(hourString).arg(minuteString);
    ...
    return QObject::tr("At %1", "Hour::Minute list").arg(timeDesc);
}
...
QString CTTask::createTimeFormat() const {
    if (hour.isAllEnabled()) {
        ...
        if (minutePeriod == 1)
            return QObject::tr("Every minute");
        else if (minutePeriod != 0)
            return QObject::tr("Every %1 minutes").arg(minutePeriod);
    }
