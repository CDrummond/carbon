...
CTCron::CTCron(const QString& crontabBinary, const struct passwd* userInfos, bool currentUserCron, CTInitializationError& ctInitializationError) :
    ...
    QTemporaryFile tmp(QDir::tempPath() + "/synkronise_XXXXXX");
    tmp.open();
    ...
    if (initializeFromUserInfos(userInfos) == false) {
        ctInitializationError.setErrorMessage(QObject::tr("No password entry found for uid '%1'").arg(getuid()));
...
QString CTCron::exportCron() const {
    ...
    exportCron += "\n";
    QString exportInfo = QObject::tr("File generated by Carbon %1.").arg(QDateTime::currentDateTime().toString());
...
CTSaveStatus CTCron::prepareSaveStatusError(const CommandLineStatus& commandLineStatus) {
    ...
    if (commandLineStatus.standardOutput.isEmpty())
        standardOutput = QObject::tr("<em>No output.</em>");
    else
        standardOutput = commandLineStatus.standardOutput;

    QString standardError;
    if (commandLineStatus.standardError.isEmpty())
        standardError = QObject::tr("<em>No error.</em>");
    else
        standardError = commandLineStatus.standardError;

    QString detailError = QObject::tr("<p><strong>Command:</strong> %1</p><strong>Standard Output :</strong><pre>%2</pre>"
                                      "<strong>Error Output :</strong><pre>%3</pre>").arg(commandLineStatus.commandLine).arg(standardOutput).arg(standardError);

    return CTSaveStatus(QObject::tr("An error occurred while updating crontab."), detailError);
}
...
CTSaveStatus CTCron::save() {
    ...
    if (saveStatus == false) {
        return CTSaveStatus(QObject::tr("Unable to open crontab file for writing"), QObject::tr("The file %1 could not be opened.").arg(d->tmpFileName));
    }
